...
import {
  selectMuscleGroupOptions,
  selectSelectedMuscleGroup
} from "../../redux/muscle-group/muscle-group.selectors";
import {
  fetchMuscleGroups,
...
class CreateWorkoutPage extends React.Component {
  ...
  handleOptionChange = option => {
    ...
    const pressed = this.props.muscleGroupOptions.find(
      option => option.isPressed === true
    ).name;
      ...
      <AbsoluteWrapper>
        <div className="create-workout-page">
          ...
          <div className="exercises-container">
            <ExercisesListWithSpinner
              isLoading={isExercisesFetching}
              {...this.props}
            />
          </div>
          {workoutExercises.length > 0 ? <NextButton path={"/circuits"} /> : ""}
        </div>
      </AbsoluteWrapper>
    );
  }
...
const mapDispatchToProps = dispatch => ({
  fetchExercisesStartAsync: muscleGroup =>
    dispatch(fetchExercisesStartAsync(muscleGroup)),
